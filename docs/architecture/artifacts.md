...
Stages write deterministic inputs, the canonical `review-result.json`, its write-once `final-findings.json` projection, posting results, and `run-summary.json`. `multi_stage` may retain private fragment documents under `raw/` only when `DEBUG_INTERMEDIATES=1`; they are not stable artifacts.
