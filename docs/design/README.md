...
| [`work-item-verification-false-positives.md`](work-item-verification-false-positives.md) | Root-cause analysis: why work-item verification produces false positives in the current 11-stage pipeline (work item / thread context is fetched but never loaded into the in-memory stage context). Includes concrete false-positive vectors, evidence trail, and the smallest correct fix. |
