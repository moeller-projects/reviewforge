...
def run_git(cwd: Path, *args: str, check: bool = True) -> str:
    cp = subprocess.run(["git", *args], cwd=str(cwd), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if check and cp.returncode:
        raise SystemExit(f"[review][ERROR] git {' '.join(args)} failed: {cp.stderr.decode(errors='replace')}")
...
def run_logged(desc: str, cmd: list[str], cwd: Path) -> None:
    ...
    for stream in (cp.stdout, cp.stderr):
        for line in stream.decode(errors='replace').splitlines():
            print(f"[review][{desc}] {line}", file=__import__('sys').stderr)
    if cp.returncode:
...
def prepare_repo(cfg: Config, source_branch: str, target_branch: str) -> RepoState:
    ...
    repo_url = f"https://dev.azure.com/{urllib.parse.quote(cfg.ado_org)}/{urllib.parse.quote(cfg.ado_project)}/_git/{urllib.parse.quote(cfg.ado_repo_id)}"
    ...
    subprocess.run(["git", "config", "--global", "--add", "safe.directory", str(repo_dir)], cwd=str(repo_dir))
    target_ref, source_ref = "refs/pr-review/target", "refs/pr-review/source"
    run_logged("git fetch target", ["git", "fetch", "--no-tags", "--depth=200", "origin", f"+refs/heads/{target_branch}:{target_ref}"], repo_dir)
    run_logged("git fetch source", ["git", "fetch", "--no-tags", "--depth=200", "origin", f"+refs/heads/{source_branch}:{source_ref}"], repo_dir)
    if subprocess.run(["git", "merge-base", target_ref, source_ref], cwd=str(repo_dir), stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL).returncode:
        run_logged("git fetch deepen target", ["git", "fetch", "--no-tags", "--deepen=1000", "origin", f"+refs/heads/{target_branch}:{target_ref}"], repo_dir)
        run_logged("git fetch deepen source", ["git", "fetch", "--no-tags", "--deepen=1000", "origin", f"+refs/heads/{source_branch}:{source_ref}"], repo_dir)
    ...
    log(f"target {target_branch} -> {target_commit}"); log(f"source {source_branch} -> {source_commit}"); log(f"merge-base -> {base}")
    ...
    files = [l for l in run_git(repo_dir, "diff", "--name-only", "--no-ext-diff", range_spec).splitlines() if l]
    return RepoState(repo_dir, source_branch, target_branch, base, source_commit, target_commit, diff, files, range_spec, cleanup_paths)
